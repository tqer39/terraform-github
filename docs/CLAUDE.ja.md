...
| タスク | コマンド |
| ------ | -------- |
| セットアップ | `./scripts/bootstrap.sh && mise install && mise run setup` |
| フォーマット | `mise run tf:fmt` |
| 検証 | `mise run tf:validate` |
| プラン | `mise run tf:plan` |
| リント | `mise run dev:lint` |
| クリーン | `mise run tf:clean` |
...
```bash
mise run tf:validate                    # 設定を検証
mise run tf:plan                        # 変更をプラン（追加/変更/削除を表示）
aws-vault exec portfolio -- mise run tf:plan  # AWS認証付き
```
